...
import libhttp
from reliableudp import ReliableUdpSocket
...
def create_parser():
    ...
    parser_get.add_argument("-p", "--port", help="Specifies the port number. Default is 8080.", type=int, default=8080, required=False)
    ...
    parser_post.add_argument("-p", "--port", help="Specifies the port number. Default is 8080.", type=int, default=8080, required=False)
...
def send_http_request(req, port):
    # Open TCP socket and send the HTTP request
    sock = ReliableUdpSocket((req.host, port))
    ba = req.to_bytearray()
    print("********** Request: **********")
    # print(ba.decode("utf-8"))
    try:
        sock.connect((req.host, port))
        sock.sendall(ba)
...
def main():
    ...
    if args.command == "help":
        ...
        while redirection == True:
            response = send_http_request(req, args.port)
            if "out" in args:
                if args.out.strip() != "":
