...
def create_parser():
    ...
    parser.add_argument("-v", "--verbose", help="Prints debugging messages.", action="store_true", default=False,
                        required=False)
    ...
    parser.add_argument("-d", "--dir",
                        help="Specifies the directory that the server will use to read/write requested files. Default "
                             "is the current directory when launching the application.",
                        type=str, default="./", required=False)
    parser.add_argument("-arq", "--autorepreq", help="....", default=False, required=False)
...
def create_arq_file_server(host, port, dir, log):
    ...
    try:
        ...
        server_sock.listen(20)
        if log:
            print('ARQ file server is listening to port = ', port)
        while True:
            client_sock, client_host = server_sock.accept()
            threading.Thread(target=incoming_connection, args=(client_sock, client_host, dir, log)).start()
    finally:
...
def main():
    ...
    if args.arq:
        create_arq_file_server('', args.port, args.dir)
    else:
        create_http_file_server('', args.port, args.dir, args.verbose)
